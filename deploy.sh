...
# Reset common conflict files (npm install will regenerate package-lock.json anyway)
if git diff --quiet package-lock.json deploy.sh 2>/dev/null; then
  # No local changes, proceed normally
  git pull origin main || echo "⚠️  Git pull failed, continuing..."
else
  echo "⚠️  Local changes detected, resetting conflict files..."
