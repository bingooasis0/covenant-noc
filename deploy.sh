...
# Reset package-lock.json if it has conflicts (npm install will regenerate it anyway)
if git diff --quiet package-lock.json 2>/dev/null; then
  # No local changes, proceed normally
  git pull origin main || echo "⚠️  Git pull failed, continuing..."
else
  echo "⚠️  Local changes detected in package-lock.json, resetting (will be regenerated by npm install)..."
